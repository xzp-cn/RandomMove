...
namespace GCSeries
{
    ...
    public class ScreenCamera3D : MonoBehaviour
    {
        ...
        { 
            ...
            string ed = Application.streamingAssetsPath + "/eyedis.txt";
            if (File.Exists(ed))
            {
                try
                {
                    eyeDis = float.Parse(File.ReadAllText(ed));
                }
                catch (Exception ex)
                {
            ...
            {
                File.WriteAllText(ed, eyeDis.ToString());
            }

            if (eyeDis > 0)
            {
                cam_l.transform.localPosition = new Vector3(-eyeDis, 0, 0);
                cam_r.transform.localPosition = new Vector3(eyeDis, 0, 0);
            }
            ResetCameraProjMat();
            this.enabled = false;
        }
