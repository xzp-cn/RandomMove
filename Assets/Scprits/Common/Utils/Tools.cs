...
public static class Tools
{
    ...
    public static void DoTrans(this Transform tr,Vector3 pos,Vector3 eular,Vector3 scale,float duration,System.Action callback)
    {    
        tr.DOLocalMove(pos, 1);
        tr.DOLocalRotate(eular, 1);
        tr.DOScale(scale, 1).onComplete=()=>{
            callback?.Invoke();
            callback = null;
        };
    }
    ...
    public static void DoTrans(this Transform tr, TransformStruct ts, float duration, System.Action callback)
    {
        tr.DOLocalMove(ts.pos, 1);
        tr.DOLocalRotate(ts.rotation.eulerAngles, 1);
        tr.DOScale(ts.localScale, 1).onComplete = () => {
            callback?.Invoke();
            callback = null;
        };
    }
...
public struct TransformStruct
{
    ...
    public Quaternion rotation;//世界rotaion
