...
public class SpliteControl : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        //---------------第二步，设置当前拆分状态------------------
        toggleTransform = this.transform;
    }
    ...
    private void Start()
    {
        ...
        {
            //把需要拆分的部件添加进来
            Transform temp= spliteObj.GetChild(i);
            //Debug.Log(temp.name);
            if (!temp.name.Contains("SpliteEnd"))
            {
        ...
        for (int i = 0; i < splitList.Count; i++)
        {
            ...
            //Debug.Log(needSpliteParts[i].name + "       -----------");
        }

        //needSpliteParts = unitObjs;
        //---------------第一步，获取拆分数据------------------
    }
    ...
    //public Transform[] unitObjs;
  
    /// <summary>
    /// 为方便吸附完成调用，才存储
    /// </summary>
    Transform toggleTransform;
    public void OnClickSpliteToggle()
    {
        //toggleTransform = toggle;
        Transform toggle = toggleTransform;
        //找到Toggle的文字
